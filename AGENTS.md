...
- **Maintenance scripts in this repository share that exemption.** `scripts/*.ps1` and `scripts/*.sh` here support the development workflow of the platform and are never deployed to or executed by any Greenhouse service. They may be committed directly to `main` on the same terms, and do not count as "non-doc files" for the bundling clause above. This applies only to `scripts/` in this repository — it is not a general licence for executable files. See branching-strategy.md.
- **Work items in this repository** (documentation and grooming tasks) follow the simplified status flow: **Todo → In Grooming → Done**. They are always executed against `main` without a feature branch. Grooming is a type of documentation work — use the `documentation` skill for both. See [work-item-tracking.md](work-item-tracking.md).
- **All new features must begin as a documentation task in this repository.** Only after the documentation task establishes the spec may implementation tasks be filed in code repositories (`services`, `ui`, `edge`, `peripherals`). See [work-item-tracking.md](work-item-tracking.md).
...
- **Only Stage 6 (Retrospective) merges a pull request to `main`, and only Stage 6 sets a board item to Done.** The test, review, and QA stages hand work over at **In Review**. A pull request is approved when its required status checks are green; there is no reviewer condition. Pulling or rebasing the task branch is a different operation that every stage still performs. See [Pull Request Ownership](workflows/feature-delivery-harness.md#pull-request-ownership) and [Merge Approval](workflows/feature-delivery-harness.md#merge-approval).
- **A defect is not a feature.** A bug report carrying reproduction, root cause, and testable acceptance criteria is already groomed and is filed straight at **Ready For Dev** without `ba-po` involvement. See [Defect Intake and Promotion](workflows/feature-delivery-harness.md#defect-intake-and-promotion).
...
4. If direction is ambiguous, use `.agents/skills/ba-po/SKILL.md` Plan Interrogation Method and ask one question at a time with a recommended answer.
