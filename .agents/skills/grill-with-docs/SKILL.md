...
Use the same Plan Interrogation Method terminology and behavior as the [documentation skill](../documentation/SKILL.md).

## Workflow

1. Read `AGENTS.md`, `CONTEXT.md`, `DOCS-MAP.md`, relevant journeys, and affected ADRs.
2. If the plan is ambiguous, run the Plan Interrogation Method.
...
8. Update `CONTEXT.md` when canonical terms are clarified.
9. Propose an ADR only when a decision is hard to reverse, surprising, and trade-off driven.
...
- Call out terminology drift against `CONTEXT.md`.
...
- Keep `CONTEXT.md` glossary-only and free of implementation detail.
- Keep ADRs concise and decision-focused.
- Create files lazily only when there is concrete content to record.
- Ensure ADRs are summarized in a central ADR digest so agents can find relevant ADRs without reading each ADR in full.
