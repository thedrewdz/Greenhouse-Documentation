...
5. Entry gate: continue only when status is `ready-for-review` or `review-in-progress`.
...
13. Emit documentation feedback items for recurring issues in `.agent-output/specs/<spec-name>/doc-feedback.md` using `templates/doc-feedback.md`.
14. If blocking findings exist, set status to `ready-for-implementation`; otherwise set status to `ready-for-qa`. Append Status History in `spec-status.md`.
15. Once review is complete, decide merge safety for the active pull request.
16. If not safe to merge to `main`, record concrete implementation feedback for resolution and do not merge.
17. If safe to merge to `main`, merge the pull request to `main`.
...
- Status transition is recorded in `spec-status.md` (`ready-for-implementation` on blocking findings, otherwise `ready-for-qa`).
